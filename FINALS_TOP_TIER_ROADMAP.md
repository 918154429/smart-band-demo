...
- 当前 Q1-S 合并基线：`master` / `7a99c8a19049d4a7f06538424e10df66e0a3d2ee`
...
Q0、Q1-V、Q1-C 与 Q1-S 已全绿。下一独立切片只进入 Q2 的第一段：表盘 registry 与
现有 Lotus 表盘迁移。开场指令应是：

```text
读取 FINALS_TOP_TIER_ROADMAP.md 与 NEXT_SESSION_HANDOFF.md。
只推进 Q2 第一段：建立 watch_face_ops_t 与 registry，把现有 Lotus 表盘原样迁入
face_lotus.c，并用 host/fake LVGL/native golden 证明零视觉与行为回归。不要同时实现
Activity、Minimal、picker、Workout、History、通知、BLE 或其他 A-G 功能。
```

Q2 必须复用 Q1-S 的 settings record 与 A/B store，不再发明第二套持久化。开发板若提前
到货，可以另开 HW0 只读清点对话，但不改变软件主线，也不自动获得烧录授权。
