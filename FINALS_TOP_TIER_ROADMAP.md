...
状态：步骤 1-7 的主机侧产品接线、explicit-length UTF-8 ingress、跨队列顺序、effect
ACK/retry、DND、overlay、Center、全屏 Call 与 Workout non-blocking Call 均已实现并通过
Host/Browser。四场景 native harness、ELF/linker-map 与 stack coloration 采证路径也已实现，
但尚未取得 fresh OpenVela artifact 和 reviewed native PNG，因此 C Gate 继续保持红色。
实现阶段说明见 `docs/q4-native-gate-harness-20260723.md`。
