...
Q0、Q1-V 与 Q1-C 已全绿。下一独立切片只进入 Q1-S：versioned storage codec 与 fault
backend。开场指令应是：

```text
读取 FINALS_TOP_TIER_ROADMAP.md 与 NEXT_SESSION_HANDOFF.md。
只推进 Q1-S：先冻结 storage header、record type、schema version、CRC 和 generation，
再实现 memory backend 与可测试 file backend，并用 A/B slot 或经证据确认的原子提交
语义覆盖短写、EIO、ENOSPC、EROFS、截断、CRC、旧版本和写中断。先写纯 C golden vectors
与故障矩阵，再接 runtime。不要开始表盘、Workout、History UI 或其他 A-G 功能。
```

Q1-S 必须保持与现有 `smart_band_storage_t` 接口兼容；不得把 host memory/file backend
误报为真机文件系统原子性证据。开发板若提前到货，可以另开 HW0 只读清点对话，但不
改变软件主线，也不自动获得烧录授权。
