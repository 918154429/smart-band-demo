...
- 固定 Q1-V 旅程之外的全页面 native 像素、第二基准分辨率和长时视觉回归。
- 已证明的一次 native 滑动之外，弹层、所有应用按钮和复杂手势冲突的端到端行为。
...
| 任务 | 当前状态 | 结论 |
| --- | --- | --- |
| A 多表盘 | 未完成 | 只有一套 face build/render 路径 |
| B 运动记录 | 部分基础 | 有步数与秒表，但没有运动会话、距离、卡路里和摘要 |
| C 消息提醒 | 未完成 | 没有通知队列、弹层、来电页和震动桥接 |
| D 持久化/历史 | 未完成 | 状态只存在 RAM，没有版本化存储和趋势页 |
| E 低功耗 | 未完成 | 没有 ACTIVE/DIM/OFF 状态机和平台功耗控制 |
| F 蓝牙同步 | 未完成 | 没有协议、transport 或 BLE 模块 |
...
- [ ] Q1-C runtime/event/platform testability
- [ ] Q1-S versioned storage/fault backend
- [ ] Q2 A 三表盘闭环
...
### 对话 2：runtime/event/clock/capabilities

- 目标：Q1-C 第一半。
- 建立中央 runtime、事件队列、时钟和能力模型。
- 保证现有 UI 行为不变。

### 对话 3：storage codec 与 fault backend

- 目标：Q1-S。
- 完成版本、CRC、A/B slot、memory/file backend 和故障矩阵。
- 不先做历史页面。

### 对话 4：A 表盘 registry 与现有表盘迁移

- 先迁移 Lotus，证明无视觉/行为回归。
- 再进入新表盘实现。

### 对话 5：A Activity/Minimal、picker 与 settings

- 完成 A 垂直闭环和 100 次切换压力。

### 对话 6：step normalizer 与 workout model
...
### 对话 10：notification model/center/overlay
...
### 对话 12：power model、dim/off 与 wrist

- 完成 E 软件 Gate；真机功耗保持待验证。

### 对话 13：sync protocol 与 golden vectors
...
Q0 与 Q1-V 已全绿。2026-07-20 已实现“对话 2”的代码切片：central runtime、首批
事件类型与定长优先级队列、可注入墙钟/单调钟、capability snapshot，以及
model -> sensor -> app tick controller。MSVC `/W4 /WX` 六组生产 C、完整 UI 链接、
Browser `6/6`、evidence harness 和 shell rollback 本地通过；本机无 GCC，因此新增生产
代码 `>=90%` / 总体 `>=85%` 覆盖率门禁和真实 openvela 构建仍需在 Linux/CI 复核。

下一对话先关闭这两个未确认 Gate；通过后只进入 Q1-C 后半。开场指令应是：

```text
读取 FINALS_TOP_TIER_ROADMAP.md 与 NEXT_SESSION_HANDOFF.md。
先在 Linux/CI 运行 tests/test_core_coverage.py，确认新增 runtime 生产代码 >=90%、总体
>=85%，并用当前工作树完成一次真实 openvela 构建；失败则只修 Q1-C 第一半。两项通过
后推进 Q1-C 后半：dirty flags、storage/power/haptic/sync 可注入 no-op/loopback 接口，
以及 fake LVGL 第 N 次创建失败注入。不要进入 Q1-S 或开发 A-G 页面。
```

不得仅凭本地 MSVC 结果把 Q1-C 第一半标绿；覆盖率和 openvela 构建证据补齐后，再完成
dirty flags、可注入 platform no-op/loopback 与 fake LVGL 失败注入。Q1-C Gate 全绿后，
下一独立对话进入 Q1-S。开发板若提前到货，可以另开
硬件 HW0 清点对话，但不改变软件主线，也不自动获得烧录授权。
