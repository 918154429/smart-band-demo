...
开发板采购候选暂定为润芯微 `Gemini-S1`（全志 R528）。公开资料已确认该型号通过
openvela 5.2 兼容性认证并存在 `dev-ai-contest-2026` BSP，但下单/到货状态、实物 SKU、
PCB revision、配套 manifest 和恢复工具仍未确认。在完成板卡清点、恢复方案和本次
烧录授权前，禁止写入 flash、烧录固件或更改 bootloader。板卡决策与能力缺口见
...
| B 运动记录 | 完成 | Walk/Run service/UI、异常恢复、三启动 native 与 30 分钟资源 soak 全绿 |
| C 消息提醒 | pure core ready | 固定通知模型/demo 已有；center/overlay/call/haptic 未接 |
| D 持久化/历史 | 完成 | 30 天 daily/session、7 天趋势、transaction recovery 与 native restart 全绿 |
...
| G 开发板适配 | 候选已定，未开始 | 暂定 Gemini-S1；公开 BSP/defconfig 已定位，实物 revision、固定基线和 board port 未确认 |
...
的 `#ifdef`。Gemini-S1 的公开 defconfig 只能证明配置存在；完成实物清点和 capability
matrix 前，不得把板载传感器、震动器、BLE 控制器或对应 driver 记为真机已通过。
...
openvela_app/smart_band/
  ...
  platform/
    ...
    board/gemini_s1/...        # HW0 冻结实物 revision 后创建
  ui/lvgl/
    controller.c
    router.c
    overlay.c
...
目标板暂定为 Gemini-S1。到货前可以执行不写硬件的 G0：冻结 contest manifest/BSP
commit、审计 `nsh_minidisplay`、把应用做 compile/link-only 接入；不得修改现有
`trunk-5.4` goldfish 固定基线来假装板卡构建。到货后按第 9 节的硬件流程执行。
...
该入口使用 Windows OpenSSH、专用 `id_ed25519_cloudlinux` key 连接
`ubuntu@sapiens.homono.de:28200`，并从 `/home/ubuntu` 启动远端 Codex。key 文件只作本机
认证，不得读取或复制到仓库。需要非交互执行 Linux 编译、coverage 或只读环境探测时，
使用同一 SSH 入口；例如：
...
if (-not $ssh) { throw 'No usable ssh.exe was found' }
& $ssh -p 28200 `
  -i "$env:USERPROFILE\.ssh\id_ed25519_cloudlinux" `
  -o IdentitiesOnly=yes -o StrictHostKeyChecking=accept-new `
  ubuntu@sapiens.homono.de `
  'uname -a; command -v gcc; command -v clang; df -h /data'
```

**固定工具链规则：Windows 本机缺少 GCC、Clang、gcovr 或其他 Linux 工具时，不得直接
把对应 Gate 记为“无法验证”，也不得在后续对话反复寻找本机 Linux 工具链；先通过上述
远端入口探测并在远端执行。** 远端 Linux 是本项目已提供的正式验证资源，GitHub Actions
仍作为独立 CI 复核，不能用 Windows MSVC 结果替代 Linux GCC/Clang/coverage。

远端约有 `150G` 数据盘。所有新的 source checkout、venv、build、cache 与 evidence 必须
放在 `/data` 下的时间戳隔离目录；大型拉取或构建前先用 `df -h /data`、`lsblk`、`pwd` 和
相关进程检查确认现场。`Remote Codex.cmd` 中的 `cd /home/ubuntu` 仅是 Codex 启动目录，
不授权在 `/home` 创建新项目工作区。旧
`/home/ubuntu/smart-band-sim-20260720-v1` 仅为历史 artifact/dirty-tree snapshot，不作为
当前源码、coverage 或 fresh build 依据，也不得覆盖或继续写入。
...
门禁。Q3 已在此基础上接入 runtime、storage、service 与 UI，最终证据见
`docs/q3-workout-history-20260722.md`。
...
状态：已完成。Host/Browser/coverage、三启动非空 file backend、1437 项证据哈希、七张
reviewed native PNG，以及 5+30 分钟 heap/fd 定向 soak 全绿；结构化结论见
...
状态：已完成。Host/Browser、1000 条混合/重复通知与满载策略、explicit-length UTF-8
ingress、DND、overlay、Center、全屏 Call、simulated haptic 与 Workout non-blocking Call
均通过；fresh 固定 OpenVela 四场景为 24/24 checks，336+167 项两级 manifest 零失败，
8/8 native PNG 已逐张人工复核，最低 stack 余量 53.840%，ELF/System.map/linker map
一致。结构化结论见 `docs/evidence/q4-gate-summary-20260724.json`，人工审计见
`docs/q4-notification-native-gate-20260724.md`。当前字体的非 ASCII 内容仍显示为 tofu，
故不声明 Unicode 字形可读；Goldfish/simulated haptic/synthetic wake 也不声明真机、
真实震动、真实功耗或 BLE。本次是 Q4 专项短 journey，不替代 8.2 的 30 分钟定向
soak、模拟器 RC 或 Q8 长稳/交互 Gate，后三者继续保持开放。
...
- Gemini-S1 实物无线模组、HCI 通道与 openvela ZBlue/GATT 配置确认后实现。
...
硬件到货前可与 Q2–Q6 的纯逻辑工作并行启动 G0 compile-only；硬件到货后才能启动
HW0/HW1。板卡业务集成必须复用已经冻结的 runtime/platform/transport 接口。

G0 必须同时满足：

- 保留 goldfish `trunk-5.4` 作为模拟器回归基线。
- 为 Gemini-S1 单独冻结包含 `r528s3-gemini-s1` 的 manifest 与 40 位项目 SHA。
- 从 `nsh_minidisplay` 派生最小配置并证明 `smart_band` compile/link，不烧录、不声称真机运行。
- 记录 BSP 与默认 `dev`/`trunk-5.4` 的差异，禁止依赖浮动分支作为 Gate 证据。
...
| Gemini-S1 board | display/touch/RTC/storage/sensor/haptic/BLE/power | 精确 revision 与固定 BSP 下冷启动、断电恢复、全功能、功耗和长稳通过 |
...
当前候选暂定为 Gemini-S1；仍必须以实物完整商品名、SKU、PCB revision 和原厂资料为准。
公开认证、README 或 defconfig 不能替代同一块实物的驱动、恢复与外设验证。
...
以下为 Gemini-S1 实物清点前的 provisional budget；板卡资源确认后只能收紧或有证据地调整。
...
| Gemini-S1 BSP 仅在 contest 分支 | 与 goldfish `trunk-5.4` 漂移，回归结论混淆 | 双基线、固定 manifest/项目 SHA、分别留 build receipt |
| 2.8 寸屏方向/分辨率未实测 | 现有 `336x480` UI 裁切或触控映射错误 | HW2 先读 framebuffer/输入参数，再补目标分辨率 native E2E |
| 板载环境传感器不等于健康传感器 | 心率、步数、运动数据只能回退模拟值 | HW4 逐节点清点，缺失能力外挂或明确降级 |
...
- [x] Q2 A 三表盘闭环
- [x] Q3 B Walk/Run 完整运动闭环
- [x] Q3 D 30 天存储与 7 天趋势闭环
- [x] Q4 C 通知/来电/haptic 闭环
- [ ] Q5 E dim/off/wrist/power policy 闭环
- [ ] Q6 F protocol + loopback
- [ ] Q6 F Linux client + real BLE
- [ ] Q7 G0 Gemini-S1 固定 BSP compile/link-only
- [ ] Q7 G 板卡 HW0–HW9
- [ ] Q8 模拟器 RC
- [ ] Q8 真机 RC
...
- 到货前可单独启动 G0 固定 BSP compile/link-only，不写硬件。
- Gemini-S1 一到即可单独启动 HW0 清点。
...
Q3 必须复用 Q1-S 的 A/B store 与 W1 workout pure core。Gemini-S1 候选板若到货，可以
另开 HW0 只读清点对话，但不改变软件主线，也不自动获得烧录授权；到货前只允许另开
G0 compile-only 对话。
...
- 模拟器与真机证据严格分层。
- 采购与适配候选暂定为 Gemini-S1；goldfish 与板卡使用独立固定基线。
- 板卡烧录需要逐次明确授权。

### Gemini-S1 已有公开证据

- openvela 官方认证表：Rivotek Gemini-S1、全志 R528 双核 Cortex-A7、认证版本 5.2。
- 公开 BSP：`vendor/allwinnertech/boards/r528/r528s3-gemini-s1`，当前位于
  `dev-ai-contest-2026`，含 `nsh`、`nsh_minidisplay` 与 `bootloader`。
- `nsh_minidisplay` 公开配置包含 LVGL/NuttX/libuv、ILI9341、GT911 touch、uORB、
  Wi-Fi、Bluetooth LE/GATT；这些是配置证据，不是真机通过结论。
- 公开硬件说明列出 2.8 寸 SPI 屏、7 寸 MIPI 扩展、温湿度/光照/接近传感器；当前
  未确认板载心率、step counter、accelerometer、电池计量或震动器。

### Gemini-S1 实物仍待确认

- 完整商品名、SKU、PCB revision、RAM/Flash 实际布局和配套 manifest/BSP commit。
- display/touch 的实际控制器、分辨率、方向、设备节点与输入坐标。
- 板载/外挂传感器、PMIC、电池、充电器、震动器型号和设备节点。
- BLE controller、HCI 与 openvela stack。
- flash 文件系统、分区与原子 rename 能力。
- 绝对 RAM/flash/stack/power budgets。
- 是否能 RAM boot、读回原厂镜像和完整恢复。

HW0 后必须在 `platform/board/gemini_s1/` 或等价位置保留 capability matrix、构建配置、
恢复说明和 board-specific technical notes；HW0 前不创建伪适配代码。新增 Bleak、字体、
图标或其他依赖时维护技术依赖清单、版本、来源和许可证信息；该要求属于工程可维护性，
不安排平台提交事务。
