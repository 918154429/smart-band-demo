...
- 三款表盘：Lotus Health 显示健康摘要，Activity Rings 突出活动进度，Minimal Digital
  突出时间、日期和电量；600ms 长按可进入表盘选择器。
...
- 基础交互：支持左右滑动切换表盘页、心率、计步和应用中心页面；应用中心点击图标
  进入不同小应用。
...
- 表盘选择持久化：selected face 使用独立 A/B settings record，支持旧版本迁移、损坏回退
  和写入失败时的安全降级；无 storage 时只保留本次会话选择。
...
- 表盘页长按 600ms：打开表盘选择器；Previous/Next 或横滑切换预览，Apply 应用，Cancel
  保持当前表盘。
- 应用中心：点击图标进入小应用。
