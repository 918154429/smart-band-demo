...
static int test_registry_contract(void)
{
  ...
  CHECK(descriptor->id == SMART_BAND_WATCH_FACE_LOTUS);
  CHECK(strcmp(descriptor->name, "Lotus") == 0);
  CHECK(descriptor->context_size > 0);
  CHECK(descriptor->context_size <= SMART_BAND_WATCH_FACE_CONTEXT_CAPACITY);
...
static int run_lotus_failure_sweep(bool compact_band)
{
  smart_band_watch_face_instance_t instance;
  ...
  CHECK(fake_lvgl_find_text(page, "Sleep", 0) != NULL);
  CHECK(fake_lvgl_find_text(page, "Heart Rate", 0) != NULL);
  CHECK(fake_lvgl_find_text(page, "Stress", 0) != NULL);
  CHECK(fake_lvgl_find_text(page, "Weather", 0) != NULL);
  smart_band_watch_face_unmount(&instance);
  CHECK(resources_are_zero());
...
int main(void)
{
  ...
  CHECK(run_lotus_failure_sweep(true) == 0);
  CHECK(run_lotus_failure_sweep(false) == 0);
  return 0;
}
