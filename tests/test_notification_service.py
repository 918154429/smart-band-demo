"""Compile and execute the standalone notification service tests."""
...
TEST_SOURCE = Path(__file__).with_name("notification_service_test.c")
PRODUCTION_SOURCES = [
...
def compile_and_run() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="smart-band-notification-service-") as temp:
        build_dir = Path(temp)
        executable = build_dir / (
            "notification_service_test.exe" if os.name == "nt" else
        ...
        if family == "msvc":
            command = [
                ...
                f"/Fo{temp}{os.sep}", f"/Fe:{executable}",
            ]
        else:
            command = [
        ...
        print("compiling notification service:", " ".join(command), flush=True)
        if compiler_environment is None:
            subprocess.run(command, cwd=ROOT, check=True)
        else:
            batch = build_dir / "compile_notification_service_test.bat"
            batch.write_text(
                "@echo off\r\n"
...
def main() -> None:
    for source in [TEST_SOURCE, *PRODUCTION_SOURCES]:
        if not source.is_file():
            raise RuntimeError(f"notification service source is missing: {source}")
    compile_and_run()
...
if __name__ == "__main__":
    try:
        ...
        print(f"notification service test failed: {error}", file=sys.stderr)
