"""Compile and execute focused workout-service production tests."""
...
TEST_SOURCE = Path(__file__).with_name("workout_service_test.c")
PRODUCTION_SOURCES = [
...
def compile_and_run() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="smart-band-workout-service-") as temp:
        build_root = Path(temp)
        output = build_root / (
            "workout_service_test.exe" if os.name == "nt" else "workout_service_test"
        )
        ...
        print("compiling workout service tests:", " ".join(command), flush=True)
        if compiler_environment is None:
            subprocess.run(command, cwd=ROOT, check=True)
        else:
            batch = build_root / "compile_workout_service_test.bat"
            batch.write_text(
                "@echo off\r\n"
            ...
            subprocess.run(
                ["cmd.exe", "/d", "/c", str(batch)], cwd=ROOT, check=True
            )
...
if __name__ == "__main__":
    try:
        ...
        print(f"workout service test failed: {error}", file=sys.stderr)
