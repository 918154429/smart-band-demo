"""Compile and execute the standalone step normalizer and workout model tests."""
...
def compile_and_run() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="smart-band-workout-core-") as temp:
        output = Path(temp) / ("workout_core_test.exe" if os.name == "nt" else "workout_core_test")
        sources = [str(TEST_SOURCE), *(str(source) for source in PRODUCTION_SOURCES)]
        if family == "msvc":
            command = [
                compiler, "/nologo", "/std:c11", "/W4", "/WX",
                f"/I{INCLUDE_DIR}", *sources, f"/Fo{temp}{os.sep}", f"/Fe:{output}",
            ]
        else:
            command = [
        ...
        print("compiling workout core:", " ".join(command), flush=True)
        if compiler_environment is None:
            subprocess.run(command, cwd=ROOT, check=True)
        else:
            batch = Path(temp) / "compile_workout_core_test.bat"
            batch.write_text(
                "@echo off\r\n"
...
if __name__ == "__main__":
    try:
        ...
        print(f"workout core host test failed: {error}", file=sys.stderr)
