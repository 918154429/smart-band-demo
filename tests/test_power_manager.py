"""Compile and execute the production power-manager service test."""
...
TEST_SOURCE = Path(__file__).with_name("power_manager_test.c")
PRODUCTION_SOURCES = [
...
def find_compiler() -> tuple[str, str, Path | None]:
    ...
    if requested:
        ...
        if resolved:
            ...
            return resolved, "msvc" if name in {"cl", "cl.exe"} else "unix", None
...
def compile_and_run() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="smart-band-power-manager-") as directory:
        temp = Path(directory)
        output = temp / ("power_manager_test.exe" if os.name == "nt" else "power_manager_test")
        ...
        print("compiling production power manager:", " ".join(command), flush=True)
        if compiler_environment is None:
            subprocess.run(command, cwd=ROOT, check=True)
        else:
            batch = temp / "compile_power_manager_test.bat"
            batch.write_text(
                "@echo off\r\n"
...
if __name__ == "__main__":
    try:
        ...
        print(f"power manager host test failed: {error}", file=sys.stderr)
