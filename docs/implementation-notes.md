...
- 真机候选暂定为 Gemini-S1；适配前先按 `docs/gemini-s1-target-board.md` 冻结独立 BSP
  基线并确认设备节点。当前电池路径仍为 goldfish 专用，心率/step/accel 也不能由板载
  环境传感器替代；必要时再把路径做成 board config 或增加受控映射。
